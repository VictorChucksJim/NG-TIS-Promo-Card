...
const CardRenderer = (() => {
  ...
  function drawBackground(ctx,w,h){
    ...
    const glow=ctx.createRadialGradient(w*.82,h*.24,10,w*.82,h*.24,560);glow.addColorStop(0,"rgba(54,230,234,.25)");glow.addColorStop(.55,"rgba(54,230,234,.07)");glow.addColorStop(1,"rgba(54,230,234,0)");ctx.fillStyle=glow;ctx.fillRect(0,0,w,h);
    ...
    ctx.save();ctx.globalAlpha=.10;ctx.strokeStyle=C.cyan;ctx.lineWidth=3;ctx.beginPath();ctx.arc(w*.5,h*.83,610,Math.PI*1.02,Math.PI*1.92);ctx.stroke();ctx.restore();
  }
  ...
  function drawFadedMarketScene(ctx){
    ...
    ctx.globalAlpha=.14;ctx.strokeStyle=C.gold;ctx.lineWidth=11;ctx.lineJoin="round";ctx.beginPath();ctx.moveTo(690,1220);ctx.lineTo(770,1150);ctx.lineTo(825,1175);ctx.lineTo(885,1040);ctx.lineTo(940,1070);ctx.lineTo(1030,900);ctx.stroke();ctx.beginPath();ctx.moveTo(1002,900);ctx.lineTo(1030,900);ctx.lineTo(1027,930);ctx.stroke();ctx.restore();
  }

  function drawRibbon(ctx,w,h){ctx.save();const g=ctx.createLinearGradient(0,h-470,390,h-220);g.addColorStop(0,C.cyan);g.addColorStop(.5,C.gold);g.addColorStop(1,"#FFE29A");ctx.globalAlpha=.62;ctx.fillStyle=g;ctx.beginPath();ctx.moveTo(0,h-470);ctx.lineTo(390,h-245);ctx.lineTo(0,h-330);ctx.closePath();ctx.fill();ctx.restore();}
  ...
  async function render(canvas,config,participant,photoCanvas){
    ...
    const badge=await loadBadge(config.badgeSrc);const badgeW=335,badgeH=badgeW*(badge.height/badge.width);ctx.drawImage(badge,68,54,badgeW,badgeH);
    ...
    // Primary event message is centred.
    ctx.fillStyle=C.cyan;ctx.font="600 27px Poppins, sans-serif";centerText(ctx,"I'M ATTENDING",w/2,342);
    ...
    const leadFit=fitText(ctx,lead,920,700,66,44);ctx.fillStyle=C.gold;ctx.font=`700 ${leadFit.size}px Poppins, sans-serif`;centerText(ctx,leadFit.lines[0],w/2,405);
    const restFit=fitText(ctx,rest,900,600,48,32);ctx.fillStyle=C.white;ctx.font=`600 ${restFit.size}px Poppins, sans-serif`;restFit.lines.forEach((line,i)=>centerText(ctx,line,w/2,478+i*restFit.size*1.16));

    // Event details balance the centred participant rather than competing with it.
    ...
    ctx.fillStyle=C.gold;ctx.font="600 22px Poppins, sans-serif";ctx.textAlign="right";ctx.fillText("SECOND EDITION",1000,688);ctx.fillStyle=C.muted;ctx.font="400 20px Poppins, sans-serif";ctx.fillText("NICEGENE TECH INSIGHT SERIES",1000,724);

    // Participant photo and name are centred.
    const photoX=w/2,photoY=850,photoSize=390;
    ...
    const nameFit=fitText(ctx,participant.name||"",700,700,58,34);ctx.fillStyle=C.white;ctx.font=`700 ${nameFit.size}px Poppins, sans-serif`;nameFit.lines.forEach((line,i)=>centerText(ctx,line,photoX,1080+i*nameFit.size*1.1));

    // QR is centred and always encodes the configured registration page.
    const qrSize=190,qrTop=1150,qrX=photoX-qrSize/2;
    ...
    // Side messages support the central QR without competing with it.
    ctx.fillStyle=C.cyan;ctx.font="700 20px Poppins, sans-serif";ctx.textAlign="left";ctx.fillText("PRACTICAL INSIGHTS",70,1180);ctx.fillStyle=C.white;ctx.font="500 21px Poppins, sans-serif";["for smarter","investment","decisions."].forEach((t,i)=>ctx.fillText(t,70,1220+i*31));
    ctx.fillStyle=C.cyan;ctx.font="700 20px Poppins, sans-serif";ctx.textAlign="right";ctx.fillText("LEARN. CONNECT. GROW.",1010,1180);ctx.fillStyle=C.white;ctx.font="500 21px Poppins, sans-serif";ctx.fillText("Be part of the conversation.",1010,1222);

    // Main CTA sits directly below the centred QR.
    roundedRect(ctx,335,1362,410,72,36);const cta=ctx.createLinearGradient(335,1362,745,1434);cta.addColorStop(0,C.gold);cta.addColorStop(1,"#FFE39A");ctx.fillStyle=cta;ctx.fill();ctx.fillStyle=C.navy;ctx.font="700 34px Poppins, sans-serif";centerText(ctx,"REGISTER NOW",540,1394);ctx.font="600 17px Poppins, sans-serif";centerText(ctx,"FOR NTIS 002",540,1420);
    ctx.fillStyle=C.white;ctx.font="400 21px Poppins, sans-serif";centerText(ctx,"Scan the QR code to register",540,1455);

    const footerTop=h-66;ctx.fillStyle="rgba(3,15,40,.90)";ctx.fillRect(0,footerTop,w,66);ctx.strokeStyle="rgba(54,230,234,.4)";ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(0,footerTop);ctx.lineTo(w,footerTop);ctx.stroke();ctx.fillStyle=C.white;ctx.font="500 18px Poppins, sans-serif";ctx.textBaseline="middle";ctx.textAlign="left";ctx.fillText(config.website||"www.nicegeneco.com.ng",40,footerTop+33);centerText(ctx,"LEARN  |  CONNECT  |  GROW",540,footerTop+33);ctx.textAlign="right";ctx.fillText(config.email||"info@nicegeneco.com.ng",1040,footerTop+33);
    return canvas;
  }
  return {render};
})();
