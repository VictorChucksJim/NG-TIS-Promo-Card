...
const CardRenderer = (() => {
  ...
  function fitSingleLine(ctx,text,maxWidth,weight,base,min){let size=base;while(size>min){ctx.font=`${weight} ${size}px Poppins, sans-serif`;if(ctx.measureText(String(text||"")).width<=maxWidth)break;size-=1;}return{size};}
  ...
  async function render(canvas,config,participant,photoCanvas){
    ...
    const name=String(participant.name||"");const nameFit=fitSingleLine(ctx,name,760,700,58,28);ctx.fillStyle=C.white;ctx.font=`700 ${nameFit.size}px Poppins, sans-serif`;centerText(ctx,name,photoX,1080);
